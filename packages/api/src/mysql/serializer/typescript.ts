...
/**
 * Convert snapshot to TypeScript
 * @param snapshot - Snapshot
 * @param casing - Casing
 * @returns TypeScript
 */
export function snapshotToTypeScript(snapshot: Snapshot, casing: Casing) {
  ...
  const imports = Object.values(snapshot.tables).reduce(
    (res, it) => {
      const idxImports = Object.values(it.indexes).map((idx) =>
        idx.isUnique ? "uniqueIndex" : "index",
      );
      const fkImpots = Object.values(it.foreignKeys).map(() => "foreignKey");
      ...
      const columnImports = Object.values(it.columns)
        .map((col) => {
          ...
          patched = patched.startsWith("tinyint unsigned")
            ? "tinyint"
            : patched;
          patched = patched.startsWith("smallint unsigned")
            ? "smallint"
            : patched;
          patched = patched.startsWith("mediumint unsigned")
            ? "mediumint"
            : patched;
  ...
  Object.values(snapshot.views).forEach((it) => {
  ...
  const tableStatements = Object.values(snapshot.tables).map((table) => {
  ...
  const viewsStatements = Object.values(snapshot.views).map((view) => {
    const {
      columns,
      name,
      algorithm,
...
function unescapeSingleQuotes(str: string, ignoreFirstAndLastChar: boolean) {
...
const column = (
  ...
  if (lowered.startsWith("double")) {
    ...
    let out = params
      ? `${casing(name)}: double(${dbColumnName({
          name,
          casing: rawCasing,
  ...
  if (lowered.startsWith("timestamp")) {
    ...
    let out = params
      ? `${casing(name)}: timestamp(${dbColumnName({
          name,
          casing: rawCasing,
  ...
  if (lowered.startsWith("varchar")) {
    let out: string = `${casing(
      name,
    )}: varchar(${dbColumnName({ name, casing: rawCasing, withMode: true })}{ length: ${lowered.substring(
      "varchar".length + 1,
      lowered.length - 1,
    )} })`;
  ...
  if (lowered.startsWith("char")) {
    let out: string = `${casing(
      name,
    )}: char(${dbColumnName({ name, casing: rawCasing, withMode: true })}{ length: ${lowered.substring(
      "char".length + 1,
      lowered.length - 1,
    )} })`;
  ...
  if (lowered.startsWith("datetime")) {
    ...
    out = fsp
      ? `${casing(
          name,
        )}: datetime(${dbColumnName({ name, casing: rawCasing, withMode: true })}{ mode: 'string', fsp: ${lowered.substring(
          "datetime".length + 1,
          lowered.length - 1,
        )} })`
  ...
  if (lowered.startsWith("decimal")) {
    ...
    let out = params
      ? `${casing(name)}: decimal(${dbColumnName({
          name,
          casing: rawCasing,
          withMode: timeConfigParams !== undefined,
        })}${timeConfigParams})`
  ...
  if (lowered.startsWith("varbinary")) {
    ...
    let out = params
      ? `${casing(name)}: varbinary(${dbColumnName({
          name,
          casing: rawCasing,
          withMode: params !== undefined,
        })}${params})`
...
const createTableColumns = (
  ...
  columns.forEach((it) => {
    ...
    statement += it.generated
      ? `.generatedAlwaysAs(sql\`${it.generated.as.replace(
          /`/g,
          "\\`",
    ...
    if (fks) {
      const fksStatement = fks
        .map((it) => {
          ...
          return `.references(()${typeSuffix} => ${casing(it.tableTo)}.${casing(
            it.columnsTo[0],
          )})`;
        })
        .join("");
