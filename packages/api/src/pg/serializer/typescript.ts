...
import type { Casing } from "../../internal/global";
import type {
  CheckConstraint,
  PrimaryKey,
  Column,
...
export function snapshotToTypeScript(snapshot: Snapshot, casing: Casing) {
  ...
  const schemas = Object.fromEntries(
    Object.entries(snapshot.schemas).map((it) => {
      return [it[0], withCasing(it[1], casing)];
    }),
  );

  const enumTypes = Object.values(snapshot.enums).reduce((acc, cur) => {
  ...
  const imports = Object.values(snapshot.tables).reduce(
    (res, it) => {
      ...
      const policiesImports = Object.values(it.policies).map(() => "pgPolicy");
  ...
  Object.values(snapshot.sequences).forEach((it) => {
    if (it.schema && it.schema !== "public" && it.schema !== "") {
      imports.pg.push("pgSchema");
    } else if (it.schema === "public") {
  ...
  Object.values(snapshot.enums).forEach((it) => {
    if (it.schema && it.schema !== "public" && it.schema !== "") {
      imports.pg.push("pgSchema");
    } else if (it.schema === "public") {
  ...
  const enumStatements = Object.values(snapshot.enums)
    .map((it) => {
  ...
  const sequencesStatements = Object.values(snapshot.sequences)
    .map((it) => {
      ...
      return `export const ${withCasing(paramName, casing)} = ${func}("${it.name}"${params ? `, { ${params.trimChar(",")} }` : ""})\n`;
    })
    .join("")
    .concat("");
  ...
  const rolesStatements = Object.entries(snapshot.roles)
    .map((it) => {
      const fields = it[1];
  ...
  const viewsStatements = Object.values(snapshot.views)
    .map((it) => {
...
/**
 * Convert the SQL schema dump to a Drizzle TypeScript file
 *
 * **It requires `@electric-sql/pglite` and `@electric-sql/pglite/vector` to be installed**
 *
 * @param sqlDump - The SQL dump
 * @returns The TypeScript file
 */
export async function sqlToTypescript(sqlDump: string) {
  const snapshot = await sqlToSnapshot(sqlDump);
  const { file } = snapshotToTypeScript(snapshot, "preserve");
  return file;
}
