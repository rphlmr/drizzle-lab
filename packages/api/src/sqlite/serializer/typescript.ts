...
export function snapshotToTypeScript(snapshot: Snapshot, casing: Casing) {
  ...
  const viewsStatements = Object.values(snapshot.views).map((view) => {
    ...
    statement += createTableColumns(Object.values(view.columns), [], casing);
    statement += "})";
  ...
  columns.forEach((it) => {
    ...
    statement += it.generated
      ? `.generatedAlwaysAs(sql\`${it.generated.as
          .replace(/`/g, "\\`")
          .slice(1, -1)}\`, { mode: "${it.generated.type}" })`
      : "";
...
/**
 * Convert the schema SQL dump to a Drizzle TypeScript file
 *
