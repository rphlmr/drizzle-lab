...
import { projectId } from "../config/schema";
import { mapValues, originUUID } from "../internal/global";
import { relation } from "../internal/relations";

// fork: drizzle-kit/src/serializer/mysqlSchema.ts
...
export const MySqlSquasher = {
  ...
  unsquashIdx: (input: string): Index => {
    const [name, columnsString, isUnique, using, algorithm, lock] =
      input.split(";");
    const destructed = {
      name,
  ...
  unsquashView: (meta: string): SquasherViewMeta => {
    ...
    const toReturn = {
      ...
      withCheckOption:
        withCheckOption !== "undefined" ? withCheckOption : undefined,
    };
...
/* lab extension */
// squashMysqlScheme
export const squashSnapshot = (json: Snapshot): SnapshotSquashed => {
  const mappedTables = Object.fromEntries(
    Object.entries(json.tables).map((it) => {
      const squashedIndexes = mapValues(it[1].indexes, (index) => {
        return MySqlSquasher.squashIdx(index);
